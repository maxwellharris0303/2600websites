...
website_urls = quickstart.getWebsiteUrl()

index = 0
for url in website_urls:
    owner_name = getOwnerName.get_owner_name(url)
    print(f"Owner Name: {owner_name}")
    email_address = extract_company_contact_info(url)
    emails = ", ".join(email_address)
    print(emails)
    # sleep(100)
    data = []
    data.append(owner_name)
    data.append(emails)
    print(data)

    quickstart.main()
    RANGE_DATA = f'TEST!B{index + 2}:C'

    quickstart.insertData(RANGE_DATA, data)
    index += 1
